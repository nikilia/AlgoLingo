...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  // Create the vertical loop animation timeline
  const loop = verticalLoop(boxes, { repeat: -1, paused: false, draggable: true });

 // ScrollTrigger for scrolling animation
 ScrollTrigger.create({
  ...
  onUpdate: (self) => {
      if (self.direction === 1) {
          // If scrolling down (forward), pause the loop animation
          loop.pause();

          // Clear the resume timer if scrolling ends before 5 seconds
